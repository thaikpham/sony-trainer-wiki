        ...
        const categories = [
            {
                ...
                color: 'from-blue-500 to-indigo-600', bg: 'from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20', ready: true,
            },
            {
                ...
                color: 'from-violet-500 to-purple-600', bg: 'from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20', ready: false,
            },
            {
                ...
                color: 'from-emerald-500 to-teal-600', bg: 'from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20', ready: false,
            },
        ];
        return (
            <Layout>
                <div className="w-full animate-slide-up flex flex-col gap-6 mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        {categories.map((cat) => {
                            ...
                            return (
                                <button key={cat.id}
                                    onClick={() => { if (!cat.ready) return; trackFeatureUsage(`cat_${cat.id}`, cat.title); setActiveCategory(cat.id); setStep('configure'); }}
                                    className={`relative group flex flex-col items-start p-7 rounded-[28px] text-left transition-all duration-300 ring-1 ${cat.ready ? 'bg-white dark:bg-[#1d1d1f] ring-black/5 dark:ring-white/5 hover:shadow-[0_12px_40px_rgba(0,0,0,0.10)] hover:-translate-y-1 cursor-pointer' : 'bg-white/50 dark:bg-white/5 ring-black/5 opacity-60 cursor-not-allowed'}`}>
                                    {!cat.ready && (<div className="absolute top-4 right-4 bg-[#F5F5F7] dark:bg-white/10 text-[#86868b] text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">Sắp ra mắt</div>)}
                                    ...
                                    <div className="text-[11px] font-bold text-[#86868b] dark:text-slate-400 uppercase tracking-widest mb-1">{cat.subtitle}</div>
                                    <div className="flex items-center gap-2 mb-3">
                                        <div className="text-[20px] font-bold text-[#1d1d1f] dark:text-white tracking-tight">{cat.title}</div>
                                        <FeatureStar featureId={`cat_${cat.id}`} />
                                    </div>
                                    <p className="text-[13px] text-[#86868b] dark:text-slate-400 leading-relaxed mb-5">{cat.desc}</p>
                                    <div className="flex flex-wrap gap-2 mt-auto">
                                        {cat.tags.map(tag => (<span key={tag} className={`text-[11px] font-semibold px-2.5 py-1 rounded-full bg-gradient-to-r ${cat.bg} text-[#424245] dark:text-slate-300 ring-1 ring-black/5 dark:ring-white/10`}>{tag}</span>))}
                                    </div>
                                    {cat.ready && (<div className="absolute bottom-6 right-6 w-9 h-9 rounded-full bg-[#1d1d1f] dark:bg-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"><ChevronRight size={16} className="text-white dark:text-[#1d1d1f]" /></div>)}
                                </button>
                            );
                        })}
                    </div>
            ...
            <Layout>
                <div className="w-full animate-slide-up flex flex-col gap-6 mx-auto">
                    ...
                    <div className="mb-2 text-center sm:text-left px-2">
                        <h2 className="text-3xl md:text-4xl font-bold text-[#1d1d1f] dark:text-white tracking-tight mb-3">Thông số Đầu vào</h2>
                        <p className="text-[15px] text-[#86868b] dark:text-slate-400 font-medium">Định chuẩn các biến số để phân tích cơ sở dữ liệu hệ sinh thái E-Mount.</p>
                    </div>

                    <div className="bg-white dark:bg-[#1d1d1f] rounded-[32px] p-6 sm:p-10 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.02] dark:ring-white/[0.05]">
                        <div className="text-[12px] font-bold text-[#86868b] dark:text-slate-400 mb-6 uppercase tracking-wider flex items-center">
                            <div className="bg-[#F5F5F7] dark:bg-white/10 p-2 rounded-xl mr-3 text-[#1d1d1f] dark:text-white"><Hexagon size={16} /></div>
                    ...
                    <div className="bg-white dark:bg-[#1d1d1f] rounded-[32px] p-6 sm:p-10 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.02] dark:ring-white/[0.05]">
                        <div className="text-[12px] font-bold text-[#86868b] dark:text-slate-400 mb-8 uppercase tracking-wider flex items-center">
                            <div className="bg-[#F5F5F7] dark:bg-white/10 p-2 rounded-xl mr-3 text-[#1d1d1f] dark:text-white"><Target size={16} /></div>
                        ...
                        <div className="flex flex-col gap-10">
                            {[
                                ...
                                <div key={prefix}>
                                    <label className="block text-[15px] font-semibold text-[#1d1d1f] dark:text-white mb-4">{label}</label>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {opts.map(opt => (
                                            <button key={`${prefix}-${opt.id}`} onClick={() => setState(opt.id)} className={`p-4 rounded-2xl text-left transition-all duration-300 ${state === opt.id ? 'bg-slate-900 dark:bg-white shadow-[0_4px_12px_rgba(0,0,0,0.1)] scale-[1.02]' : 'bg-white dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/5 hover:bg-[#F5F5F7]/50'}`}>
                                                <div className={`text-[15px] font-semibold mb-1 ${state === opt.id ? 'text-white dark:text-slate-900' : 'text-[#1d1d1f] dark:text-white'}`}>{opt.label}</div>
                                                <div className={`text-[13px] ${state === opt.id ? 'text-slate-300 dark:text-slate-600' : 'text-[#86868b] dark:text-slate-400'}`}>{opt.desc}</div>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div>
                                <label className="block text-[15px] font-semibold text-[#1d1d1f] dark:text-white mb-2">5. Lộ Trình Nâng Cấp (Tùy chọn)</label>
                                <p className="text-[13px] text-[#86868b] dark:text-slate-400 mb-4 font-medium">Bạn đang sử dụng thiết bị nào? AI sẽ ưu tiên giữ nguyên ngàm hoặc đề xuất nâng cấp phù hợp.</p>
                                <input type="text" value={currentGear} onChange={(e) => setCurrentGear(e.target.value)} placeholder="VD: Sony A6400, Kit 16-50mm..." className="w-full px-5 py-4 bg-[#F5F5F7] dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/10 rounded-2xl text-[15px] text-[#1d1d1f] dark:text-white placeholder:text-[#86868b] focus:outline-none focus:ring-2 focus:ring-[#1d1d1f]/10 transition-all" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#1d1d1f] rounded-[32px] p-6 sm:p-10 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.02] dark:ring-white/[0.05]">
                        <div className="text-[12px] font-bold text-[#86868b] dark:text-slate-400 mb-8 uppercase tracking-wider flex items-center">
                            <div className="bg-[#F5F5F7] dark:bg-white/10 p-2 rounded-xl mr-3 text-[#1d1d1f] dark:text-white"><SlidersHorizontal size={16} /></div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                            {[{ id: 'newbie', label: 'Nhập Môn', desc: 'Entry-level' }, { id: 'advanced', label: 'Bán Chuyên', desc: 'Cân bằng giá & hiệu năng' }, { id: 'professional', label: 'Chuyên Nghiệp', desc: 'Dành cho công việc' }, { id: 'hi-end', label: 'Tiên Tiến', desc: 'Công nghệ cao cấp' }, { id: 'flagship', label: 'Đỉnh Cao', desc: 'Không thỏa hiệp' }].map(opt => (
                                <button key={`exp-${opt.id}`} onClick={() => setExperienceLevel(opt.id)} className={`p-4 rounded-2xl text-left transition-all duration-300 ${experienceLevel === opt.id ? 'bg-slate-900 dark:bg-white shadow-[0_4px_12px_rgba(0,0,0,0.1)] scale-[1.02]' : 'bg-white dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/5 hover:bg-[#F5F5F7]/50'}`}>
                                    <div className={`text-[14px] font-semibold mb-1 ${experienceLevel === opt.id ? 'text-white dark:text-slate-900' : 'text-[#1d1d1f] dark:text-white'}`}>{opt.label}</div>
                                    <div className={`text-[12px] ${experienceLevel === opt.id ? 'text-slate-300 dark:text-slate-600' : 'text-[#86868b] dark:text-slate-400'}`}>{opt.desc}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col-reverse md:flex-row justify-end mt-4 mb-10 gap-4">
                        <button onClick={() => setStep('category')} className="w-full md:w-auto flex items-center justify-center space-x-2 bg-white dark:bg-[#1d1d1f] text-[#1d1d1f] dark:text-white border border-slate-200 dark:border-white/10 px-8 py-5 rounded-full hover:bg-[#F5F5F7] transition-all font-medium text-[15px]">
                            <ArrowLeft size={18} /><span>Quay Lại</span>
                        </button>
                        <button onClick={runAnalysisSimulation} disabled={userNeeds.length === 0} className="w-full md:w-auto group flex items-center justify-center space-x-3 bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] px-10 py-5 rounded-full hover:bg-black transition-all duration-300 shadow-[0_4px_14px_rgba(0,0,0,0.15)] disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5">
                ...
                <div className="w-full animate-slide-up flex flex-col gap-6 mx-auto">
                    ...
                    <div className="w-full max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[50vh]">
                        <div className="relative mb-10">
                            <div className="w-24 h-24 bg-white dark:bg-white/10 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.06)] flex items-center justify-center animate-pulse">
                                <Loader2 size={36} className="animate-spin text-[#1d1d1f] dark:text-white" />
                            </div>
                        </div>
                        <h3 className="text-2xl font-bold text-[#1d1d1f] dark:text-white mb-8 tracking-tight">Đang phân tích hệ thống...</h3>
                        <div className="w-full bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[32px] p-8 border border-white/40 dark:border-white/10">
                            <div className="space-y-4">
                                {loadingLog.map((log, i) => (
                                    <div key={i} className="flex items-start">
                                        <div className="mt-1 mr-4 w-5 h-5 rounded-full bg-[#F5F5F7] dark:bg-white/20 flex items-center justify-center flex-shrink-0"><div className="w-1.5 h-1.5 rounded-full bg-[#1d1d1f] dark:bg-white"></div></div>
                                        <span className="text-[15px] font-medium text-[#86868b] dark:text-slate-400">{log}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <Layout>
                <div className="w-full animate-slide-up flex flex-col mx-auto">
                    ...
                    <div className="mb-8 md:mb-12 text-center sm:text-left flex flex-col md:flex-row md:items-start justify-between gap-6 px-2">
                        <div>
                            <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-emerald-900/30 text-emerald-700 dark:text-emerald-400 font-semibold text-[11px] px-3 py-1.5 rounded-full mb-4 uppercase tracking-wider ring-1 ring-emerald-500/20">
                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div><span>Hoàn Tất Chẩn Đoán</span>
                            </div>
                            <h2 className="text-3xl md:text-5xl font-bold text-[#1d1d1f] dark:text-white tracking-tight mb-2">Cấu Hình Đề Xuất</h2>
                            <p className="text-[#86868b] dark:text-slate-400 text-[15px] font-medium">Báo cáo dựa trên thuật toán AI.</p>
                        </div>
                        <button onClick={() => setStep('configure')} className="inline-flex self-center items-center space-x-2 bg-white dark:bg-white/10 text-[#1d1d1f] dark:text-white ring-1 ring-black/5 px-5 py-2.5 rounded-full hover:bg-[#F5F5F7] transition-all font-medium text-[13px]">
                    ...
                    <div className="mb-8 flex flex-wrap gap-2 px-2">
                        {[
                            ...
                            <span key={label} className="bg-white/60 dark:bg-white/5 text-[#424245] dark:text-slate-300 text-[13px] font-medium px-4 py-1.5 rounded-full ring-1 ring-black/[0.04] dark:ring-white/10">
                                {label}: <span className="text-[#1d1d1f] dark:text-white font-semibold">{value}</span>
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="w-full mt-8 flex flex-col gap-6">
                        <div className="bg-white dark:bg-[#1d1d1f] rounded-[32px] p-6 sm:p-10 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.02] dark:ring-white/[0.05] relative overflow-hidden">
                            <div className="text-[12px] font-bold text-[#86868b] dark:text-slate-400 mb-8 uppercase tracking-wider flex items-center">
                                <div className="bg-[#F5F5F7] dark:bg-white/10 p-2 rounded-xl mr-3 text-[#1d1d1f] dark:text-white"><Cpu size={16} /></div>
                                Góc Nhìn Từ Chuyên Gia AI
                            </div>
                            {!aiInsight && !isGeneratingInsight ? (
                                <div className="flex flex-col items-center justify-center py-10">
                                    <div className="w-16 h-16 rounded-full bg-[#F5F5F7] dark:bg-white/5 flex items-center justify-center mb-4"><Cpu size={24} className="text-[#86868b]" /></div>
                                    <p className="text-[15px] text-[#86868b] dark:text-slate-400 font-medium mb-6 text-center max-w-md">Hệ thống đã chọn lọc các sản phẩm tốt nhất. Bạn có muốn AI phân tích và giải thích lý do lựa chọn không?</p>
                                    <button onClick={handleGenerateInsight} className="flex items-center space-x-2 bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] px-6 py-3 rounded-full hover:bg-black transition-all shadow-[0_4px_14px_rgba(0,0,0,0.1)]">
                                        <span className="text-[14px] font-medium">Phân Tích Bằng AI</span><ChevronRight size={16} />
                                    </button>
                                </div>
                            ) : isGeneratingInsight ? (
                                <div className="flex flex-col items-center justify-center py-12">
                                    <Loader2 size={36} className="animate-spin text-[#1d1d1f] dark:text-white mb-4" />
                                    <p className="text-[15px] text-[#86868b] font-medium">Đang tổng hợp nhận định từ cố vấn...</p>
                                </div>
                            ) : (
                                <div className="text-[15px] font-medium text-[#1d1d1f] dark:text-white leading-relaxed space-y-4">
                                    {aiInsight.split('\n').map((line, i) => {
                                        if (!line?.trim()) return <div key={i} className="h-2"></div>;
                                        if (line.trim().startsWith('-')) return (
                                            <div key={i} className="flex items-start bg-[#F5F5F7]/50 dark:bg-white/5 p-4 rounded-2xl">
                                                <div className="mr-3 mt-1.5 text-lg">•</div>
                                                <p className="text-[#424245] dark:text-slate-300">{line.substring(1).trim()}</p>
                                            </div>
                                        );
                                        return <p key={i} className="text-[#424245] dark:text-slate-300">{line}</p>;
                                    })}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <a href="https://experience.sony-asia.com/vn/home/Workshops" target="_blank" rel="noopener noreferrer" className="bg-white dark:bg-[#1d1d1f] rounded-[32px] p-8 flex flex-col justify-between ring-1 ring-black/[0.02] group hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all h-full">
                                <div className="mb-6">
                                    <div className="text-[11px] font-bold text-[#86868b] mb-2 uppercase">Thực Chiến Cùng Chuyên Gia</div>
                                    <div className="text-xl font-bold text-[#1d1d1f] dark:text-white mb-3">Sony Offline Workshops</div>
                                    <div className="text-[14px] text-[#86868b]">Đăng ký tham gia các buổi workshop thực tế offline tại Việt Nam.</div>
                                </div>
                                <div className="w-12 h-12 bg-[#F5F5F7] dark:bg-white/10 rounded-2xl flex items-center justify-center group-hover:bg-[#1d1d1f] transition-all self-end">
                                    <Target size={20} className="text-[#1d1d1f] dark:text-white group-hover:text-white transition-colors" strokeWidth={2} />
                                </div>
                            </a>
                        </div>
                    </div>
