...
export default function LiveReportDetailModal({ report, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/[0.02] backdrop-blur-[20px] backdrop-saturate-[180%] animate-in fade-in duration-200">
            <div
                ref={modalRef}
                className="bg-background w-full max-w-4xl max-h-full rounded-[40px] shadow-2xl overflow-hidden ring-1 ring-black/5 flex flex-col animate-in zoom-in-95 duration-200"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="relative p-8 sm:p-10 border-b border-black/5 shrink-0">
                    <div className="absolute top-8 right-8 flex items-center gap-2 z-10">
                        ...
                        <button
                            onClick={onClose}
                            className="p-2 rounded-full bg-black/5 text-slate-400 hover:text-foreground transition-all hover:scale-110 active:scale-95"
                        >
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center gap-2">
                            <span className="px-3 py-1 bg-teal-500/10 text-teal-600 text-[10px] font-black uppercase tracking-widest rounded-full ring-1 ring-teal-500/20">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-8 sm:p-10">
                    <div className="flex flex-col gap-10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            <div className="flex items-center gap-4 bg-black/5 p-5 rounded-3xl ring-1 ring-black/5">
                        ...
                        {report.platforms && report.platforms.length > 0 && (
                            <div className="flex flex-col gap-4">
                                <h3 className="text-[14px] font-black text-[#1d1d1f] uppercase tracking-widest flex items-center gap-2">
                                    <Monitor size={18} className="text-teal-500" />
                                    Chi tiết theo nền tảng
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {report.platforms.map((plat, idx) => (
                                        <div key={idx} className="glass-panel p-6 rounded-[32px] ring-1 ring-black/5 relative overflow-hidden group hover:scale-[1.01] transition-all">
                                            ...
                                            <div className="flex items-center justify-between mb-5 relative z-10">
                                                <span className={`px-4 py-1.5 rounded-xl text-[12px] font-black uppercase tracking-widest bg-white shadow-sm ring-1 ring-black/5 ${plat.name === 'TikTok' ? 'text-black' :
                        ...
                        {report.note && (
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="bg-black/5 p-6 rounded-[32px] text-[14.5px] font-medium text-slate-600 leading-relaxed italic whitespace-pre-wrap ring-1 ring-black/5">
                                    {report.note}
                                </div>
                            </div>
                        )}
                        ...
                        {/* Footer Info */}
                        <div className="mt-4 pt-8 border-t border-black/5 flex flex-col sm:flex-row items-center justify-between gap-6 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-700">
...
function StatCard({ icon, label, value, color, isCurrency = false }) {
    ...
    return (
        <div className="bg-white p-5 sm:p-6 rounded-[32px] ring-1 ring-black/5 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1">
