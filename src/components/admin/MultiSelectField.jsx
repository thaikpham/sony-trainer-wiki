                        ...
                        filtered.map(s => (
                            <div key={s} className="w-full flex items-center justify-between hover:bg-slate-50 transition-colors group px-1">
                                <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleTag(s); }}
                                    ...
                                    <div className={`w-4 h-4 rounded-[4px] border flex items-center justify-center transition-colors flex-shrink-0 ${value.includes(s) ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-300 bg-white'}`}>
                                        {value.includes(s) && <Check size={12} strokeWidth={3} />}
                                    </div>
