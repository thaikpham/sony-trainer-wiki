...
export default function BadgeShowcase({ userRoleKeys = [] }) {
                ...
                return (
                    <div
                        ...
                        className={`group relative overflow-hidden rounded-[24px] p-5 transition-all duration-500 ${isUnlocked
                            ? 'bg-background shadow-[0_4px_20px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.03] dark:ring-white/10 hover:shadow-[0_20px_40px_rgba(0,0,0,0.1)] hover:-translate-y-1'
                            : 'bg-black/[0.02] dark:bg-white/[0.02] ring-1 ring-black/[0.05] dark:ring-white/[0.05] saturate-[0.1] opacity-60'
                            }`}
                    >
                        ...
                        <div className="flex items-start justify-between mb-4">
                            <div
                                className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-inner transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6 ${isUnlocked ? `bg-gradient-to-br ${role.gradient} ${role.textColor || 'text-white'}` : 'bg-slate-200 dark:bg-slate-800 text-slate-400'
                                    }`}
                            ...
                            <div className="flex flex-col items-end">
                                <span className={`text-[10px] font-black uppercase tracking-[0.1em] px-2 py-0.5 rounded-full ${isUnlocked
                                    ? (isLegendary ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30' : 'bg-slate-100 dark:bg-white/10 text-slate-500')
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className={`text-[12px] leading-relaxed line-clamp-2 ${isUnlocked ? 'text-slate-500 dark:text-slate-400' : 'text-slate-300 dark:text-slate-700'}`}>
                                {role.description}
                            </p>
                        </div>
                        ...
                        {!isUnlocked && (
                            <div className="mt-4 pt-4 border-t border-black/[0.03] dark:border-white/[0.03] flex items-center gap-1.5 text-[11px] font-bold text-slate-400">
                                <Lock size={12} />
    ...
    return (
        <div className="bg-background rounded-[32px] p-6 sm:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.02] dark:ring-white/[0.05] flex flex-col gap-8">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-amber-50 dark:bg-amber-500/20 flex items-center justify-center text-amber-600 dark:text-amber-400 ring-1 ring-amber-500/20">
                        <Trophy size={20} />
                    </div>
                    <div>
                        <h2 className="text-[20px] font-black text-foreground tracking-tight leading-tight">Hệ thống Huy hiệu</h2>
                        <p className="text-[12px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">Bộ sưu tập thành tựu cá nhân</p>
                    </div>
                </div>
            </div>
